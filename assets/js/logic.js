...
$(".clear-storage").on("click", function () {
  localStorage.setItem(
    "recipeHistory",
    JSON.stringify(recipeHistory.splice(0, recipeHistory.length))
  );
  $(".storage").empty();
});
    ...
    .then(function (data) {
      ...
      if (recipe + dietOption) {
        ...
        $("#image1").on("click", function (e) {
          ...
          localStorage.setItem(
            "currentSearch",
            e.target.getAttribute("data-receipeId")
          );
...
function historyList() {
  ...
  for (var i = 0; i < recipeHistory.length; i++) {
    var storageButton = $(
      '<button type="button" class="btn btn-primary btn-lg" id="search-history" style="margin-left: 10px;">'
    ).text(recipeHistory[i]);
